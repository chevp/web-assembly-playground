// poc-05-frost-engine-api
//
// Tiny C++ runtime that mirrors the Frost Engine orchestration model from
// nuna/nuna: declarative XML scene + per-entity Lua tick scripts driving an
// engine through a small `frost.*` Lua global. No Vulkan, no GPU — the
// renderer here is plain Canvas2D in the browser. The wasm side only owns
// scene state and Lua execution; JS reads transforms back each frame and
// draws them.
...
// Strip the optional "file://" prefix nuna uses for asset URIs.
...
// Parses runtime.xml — extracts <scene uri=".."/> and any
// <scripts><script scope="scene" uri=".."/></scripts>. Returns true on
// success and writes the resolved paths to the out params.
static bool loadRuntime(const std::string& runtimePath, std::string& outScene,
    ...
    if (!renderer) renderer = root;  // tolerate either nesting
